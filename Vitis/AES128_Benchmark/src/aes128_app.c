...
int main()
{
	...
	printf("Encryption in SW took %llu clock cycles, time taken: %llu ns.\n", 2*(tEnd - tStart), (2*(tEnd - tStart)*833)/1000);

	XTime_GetTime(&tStart);
	aes128_decrypt();
	XTime_GetTime(&tEnd);

	printf("Decryption in SW took %llu clock cycles, time taken: %llu ns.\n", 2*(tEnd - tStart), (2*(tEnd - tStart)*833)/1000);
	...
	printf("Encryption in HW took %llu clock cycles, time taken: %llu ns.\n", 2*(tEnd - tStart), (2*(tEnd - tStart)*833)/1000);
	...
	printf("Decryption in HW took %llu clock cycles, time taken: %llu ns.\n", 2*(tEnd - tStart), (2*(tEnd - tStart)*833)/1000);
